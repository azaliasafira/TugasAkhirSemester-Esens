...
//create databases
  Future<int> insertWargaLama(WargaL object) async {
    Database db = await this.initDb();
    int count = await db.insert('item', object.toMap());
    return count;
  }

  Future<int> insertWargaBaru(WargaB object) async {
    Database db = await this.initDb();
    int count = await db.insert('item', object.toMap());
    return count;
  }
...
//create databases user
  Future<int> insertUser(UserLog user) async {
    Database db = await this.initDb();
...
//update databases
  Future<int> updateWargaLama(WargaL object) async {
    Database db = await this.initDb();
    int count = await db
  ...
  Future<int> updateWargaBaru(WargaB object) async {
    Database db = await this.initDb();
    int count = await db
        .update('item', object.toMap(), where: 'id=?', whereArgs: [object.id]);
    return count;
  ...
  Future<int> updateUser(UserLog user) async {
    Database db = await this.initDb();
    int count = await db
  ...
  Future<List<WargaL>> getItemList(String iduser) async {
    var itemMapList = await select(iduser);
    int count = itemMapList.length;
    List<WargaL> itemList = List<WargaL>();
    for (int i = 0; i < count; i++) {
      itemList.add(WargaL.fromMap(itemMapList[i]));
    }
    return itemList;
  }

  Future<List<WargaL>> getItemListW(String iduser) async {
    var itemMapList = await select(iduser);
    int count = itemMapList.length;
    List<WargaL> itemList = List<WargaL>();
    for (int i = 0; i < count; i++) {
  ...
  Future<List<UserLog>> getUserList(int id) async {
    var itemMapList = await selectUser(id);
    int count = itemMapList.length;
    List<UserLog> itemList = List<UserLog>();
    for (int i = 0; i < count; i++) {
      itemList.add(UserLog.fromMapUser(itemMapList[i]));
    }
    return itemList;
  }

  Future<UserLog> getLogin(String user, String password) async {
    var dbClient = await this.initDb();
    var res = await dbClient.rawQuery(
        "SELECT * FROM user WHERE username = '$user' and password = '$password'");

    if (res.length > 0) {
      return new UserLog.fromMapUser(res.first);
    }
    return null;
  }

  Future<List<UserLog>> getAllUser() async {
    var dbClient = await this.initDb();
    var res = await dbClient.query("user");

    List<UserLog> list =
        res.isNotEmpty ? res.map((c) => UserLog.fromMapUser(c)).toList() : null;
    return list;
  }
